...
M_IN_K = 1000.0
REBUF_PENALTY = 4300  # 1 sec rebuffering -> 3 Mbps
SMOOTH_PENALTY = 1
DEFAULT_QUALITY = 1  # default video quality without agent
RANDOM_SEED = 42
RAND_RANGE = 1000000
SUMMARY_DIR = './rebuf 1000배 0726'
LOG_FILE = './rebuf 1000배 0726/log_sim_mpc_최대오차반영_rebuf_x1000'
...
# lstm_predictions = [0.32777127795308186,0.2309426152483254,0.14726066818975156,0.10571789534476604
#                     ,0.17264205930515278,0.17590446066759893,0.20165997402853536,0.21392360204141928
#                     ,0.1889273393443268,0.20329074532881425,0.07929337580271492,0.13561597966360223
#                     ,0.16409536305420705,0.19064360126688143,0.21507355431297004,0.2238810337659378
#                     ,0.19047419734008406,0.27280510163920385,0.23966786386904265,0.26061339931590993
#                     ,0.2543664498762903,0.3129411962223953,0.12298637888024629,0.24024391783861673
#                     ,0.32840217000324246,0.22422657150861697,0.2699633183445352,0.2875760690056149
#                     ,0.22105588241069915,0.20210447969741202,0.21048799366381293,0.20642923961879428
#                     ,0.2802616084357289,0.31678447113644215,0.1617669616370136,0.1799651250196754
#                     ,0.11423182720080718,0.10346085003019576,0.10982368851036448,0.13290022405749136
#                     ,0.11925424352773324,0.16749341374351143,0.16820872734502548,0.2776890382301906
#                     ,0.18753558426106245,0.2316170412685009,0.12528400507548068,0.09629590165765183]

# 빈 리스트 15개를 생성합니다 # 이건 테스트 파일 개수만큼
prediction_lists = [list() for _ in range(17)]

# 파일을 열어서 각 줄을 읽어들입니다
with open("./lstm_prediction_results/tram predictions 41~56 725 02", 'r') as lstmfile:
    for i, line in enumerate(lstmfile):
        # 각 줄을 \t로 구분하고, 두 번째 값을 float으로 변환해서 해당하는 리스트에 추가합니다
        prediction = float(line.strip().split('\t')[1])
        prediction_lists[i // 48].append(prediction)
...
def main():
    ...
    while True:  # serve video forever
        ...
        # print("shape(state) = {}".format(np.shape(state)))

        # print("state[3, -1] = {}".format(state[3, -1]))
        ...
        # print("past_errors = {}".format(past_errors))

        # pick bitrate according to MPC           
        # first get harmonic mean of last 5 bandwidths
        past_bandwidths = state[3,-5:]
        # print("past_bandwidths = {}".format(past_bandwidths))
        ...
        # print("harmonic_bandwidth = {}".format(harmonic_bandwidth))
        ...
        # print("f = ",video_count)
        print("f= ", all_file_names[net_env.trace_idx])
        future_bandwidth = prediction_lists[video_count][lstm_cursor]/(1+max_error)
        # future_bandwidth = lstm_predictions[lstm_cursor]
        lstm_cursor += 1

        # print("harmonic_bandwidth = {}".format(harmonic_bandwidth))
        # print("1+max_error = {}".format(1 + max_error))
        # print("future_bandwidth = {}".format(future_bandwidth))
        #
        # print("time now  = {}".format(time_stamp/ M_IN_K))

        past_bandwidth_ests.append(harmonic_bandwidth)
        # print("past_bandwidths 당기기 = {}".format(past_bandwidths))
