...
def main():
    ...
    while True:  # serve video forever
        ...
        preprocess_futurebandwidth.append(future_bandwidth)
        ...
        preprocess_time.append(time_stamp/M_IN_K) # 대역폭 새로 추가 됐을 때 시간 기록
        preprocess_bandwidth.append((past_bandwidths[-1]))  # 그 추가된 대역 기록
        ...
        if end_of_video:
            ...
            # with open("./bandwidth_result/bandwidth data({}).txt".format(all_file_names[net_env.trace_idx]), "w") as log_file:
            #     for time, bandwidth in zip(preprocess_time, preprocess_bandwidth):
            #         log_file.write(f"{time}\t{bandwidth}\n")

            # with open("./bandwidth_result/future bandwidth data({}).txt".format(all_file_names[net_env.trace_idx]), "w") as log_file:
            #     for time, bandwidth in zip(preprocess_time, preprocess_futurebandwidth):
            #         log_file.write(f"{time}\t{bandwidth}\n")
            ...
            if video_count >= len(all_file_names):
                ...
                print(preprocess_bandwidth)
                with open("data(파일별로 시간 초기화).txt", "w") as log_file:
                    for time, bandwidth in zip(preprocess_time, preprocess_bandwidth):
                        log_file.write(f"{time}\t{bandwidth}\n")
