...
def preprocess_data(data, timestep):
    ...
    data_scaled[:, 1] = scaler_throughput.fit_transform(data[:, 1].reshape(-1, 1)).reshape(-1)

    # 입력 데이터와 타깃 데이터 분리
    X, y = [], []
    for i in range(len(data_scaled) - timestep):
        X.append(data_scaled[i:i+timestep, 1])  # 0이 아닌 1로 인덱싱하여 throughput 값만 사용
...
for file_prefix, count in file_counts.items():
    ...
    for n in range(1, count + 1):
        ...
        # 그래프 그리기 - 원래의 스케일로 변환된 데이터
        plt.figure(figsize=(14, 5))
        plt.plot(data[-test_size:, 1], label='Actual')
        plt.plot(predicted_throughput, label='Predicted')
        plt.title(f'Inverse Transformed values - Test vs Predicted for {file_name}')
        plt.legend()
        plt.show()

        # 예측 결과 저장
        result_file_name = f"{file_prefix}_merge_result.txt"
