...
for file_prefix, count in file_counts.items():
    ...
    for n in range(1, count + 1):
        ...
        # 데이터 분할 앞부분을 test data로 사종
        test_size = int(len(X) * test_ratio)
        X_train, X_test = X[:test_size], X[test_size:]
        y_train, y_test = y[:test_size], y[test_size:]
        time_stamp_test = data[:test_size, 0]
        ...
        custom_rmsprop = RMSprop(learning_rate=0.005,
                                 rho=0.9,
                                 momentum=0.0,
                                 epsilon=1e-07,
                                 centered=False,
        ...
        # 모델 컴파일
        model.compile(optimizer=custom_rmsprop, loss='mean_squared_error')
